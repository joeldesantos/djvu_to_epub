...
tree = xml.etree.ElementTree.parse(
    'C:\\workspace\\djvu_to_epub\\input\\xml.xml')
root = tree.getroot()
...
for page in pages:
    ...
    for line in paragraph.findall('LINE'):
        for word in line:
            # print(word.text)
            x_min, y_min, x_max, y_max = [int(val) for val in
                                          word.attrib['coords'].split(',')]
            if abs(y_min - p) >= 150:
                text_list.append('\n')
            p = y_min
...
# create chapter
c1 = epub.EpubHtml(title='CRACKING the CODING INTERVIEW',
                   file_name='book.xhtml', lang='en')
...
book.toc = (epub.Link('book.xhtml', 'CRACKING the CODING INTERVIEWa',
            'CRACKING the CODING INTERVIEWb'),
